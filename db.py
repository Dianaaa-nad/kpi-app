...
def fetch_kpi_variable_categories():
    conn = get_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT category_id, category_name FROM kpi_variable_categories ORDER BY category_id")
    result = cursor.fetchall()
    cursor.close()
    return result

def fetch_kpi_detail():
    """Fetch KPI variables joined with their KPI variable categories, ordered by category then var_code."""
    conn = get_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("""
        SELECT 
